 ...
 *
 * Transport: stdio (the standard for local MCP connectors).
 */
...
/**
 * Register a tool with uniform result formatting and error handling. The handler
 * returns any JSON-serialisable value (or a string); it is wrapped into MCP
 * text content, and thrown errors become an isError result.
 */
...
const snowflake = (label: string) => z.string().regex(/^\d{17,20}$/, `${label} must be a Discord snowflake ID`);
...
tool(
  "discord_update_channel",
  {
    ...
    description:
      "Modify a channel's properties. Only the fields you provide are changed. Use parent_id=null semantics by passing an empty string to remove from a category.",
    inputSchema: {
...
tool(
  ...
  ({ guild_id, role_id, ...body }) => discord("PATCH", `/guilds/${guild_id}/roles/${role_id}`, body),
);
